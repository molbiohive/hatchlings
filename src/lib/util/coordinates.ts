...
/**
 * Convert a bp position to X coordinate on a linear map.
 */
export function bpToLinearX(bp: number, totalSize: number, mapWidth: number, marginLeft: number = 0): number {
	return marginLeft + (bp / totalSize) * mapWidth;
}

/**
 * Generate an SVG path for an arrow-shaped rectangle on a linear map.
 * strand +1 draws above the backbone, -1 below.
 */
export function linearArrowPath(
	startBp: number,
	endBp: number,
	totalSize: number,
	mapWidth: number,
	...
	const x1 = bpToLinearX(startBp, totalSize, mapWidth, marginLeft);
	const x2 = bpToLinearX(endBp, totalSize, mapWidth, marginLeft);
	const w = x2 - x1;
	const halfH = featureHeight / 2;

	if (w < arrowWidth * 2) {
		// Too small for arrow — simple rect
		return `M ${x1} ${y - halfH} L ${x2} ${y - halfH} L ${x2} ${y + halfH} L ${x1} ${y + halfH} Z`;
	}

	if (strand === 1) {
		// Arrow pointing right
		const bodyEnd = x2 - arrowWidth;
		return [
			`M ${x1} ${y - halfH}`,
			`L ${bodyEnd} ${y - halfH}`,
			`L ${x2} ${y}`,
